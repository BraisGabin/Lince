    ...
    */
        ...
        deliver:
        {
            if (Math.min(aa, bb) <= 1.0)
            {
                ...
                for (; ; )
                {
                    ...
                    if (alpha * (Math.log(alpha / (b + w)) + v) -
                        1.3862944 >= Math.log(z))
                    {
                        break deliver;
                    }
                }
                                 ...
                                 double kkin)
    {
        ...
        else
        {
            if (setup1 || setup2)
            {
                s = Math.sqrt((tn - k) * k * n1 * n2 / (tn - 1) / tn /
                                        tn);
                d = (int) (1.5 * s) + .5;
                xl = m - d + .5;
                xr = m + d + .5;
                ...
                kr = Math.exp(a - afc((int) (xr - 1))
                                        ...
                                        - afc((int) (k - xr + 1))
                                        - afc((int) (n2 - k + xr - 1)));
                lamdl = -Math.log(xl * (n2 - k + xl) / (n1 - xl + 1)
                                            / (k - xl + 1));
                lamdr = -Math.log((n1 - xr + 1) * (k - xr + 1)
                                            / xr / (n2 - k + xr));
                p1 = d + d;
                p2 = p1 + kl / lamdl;
                p3 = p2 + kr / lamdr;
                            ...
                            double logsd)
    {
        ...
        return Math.exp(normal(logmean, logsd));
    }
    ...
    public double t(double df)
    {
        ...
        {
            return normal() / Math.sqrt(chisquare(df) / df);
        }
    }
                          ...
                          double scale)
    {
        ...
        return scale * Math.pow(
                -Math.log(new Random().nextDouble()), 1.0 / shape);
    }
