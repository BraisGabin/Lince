 ...
 */
package com.braisgabin.lince.datos;

import com.braisgabin.lince.modelo.FilaRegistro;
...
public class ControladorArchivos {
    ...
    public void guardarSerializable(File path, Serializable datos) throws FileNotFoundException, IOException {
        ObjectOutputStream salida = new ObjectOutputStream(new FileOutputStream(path));
        salida.writeObject(datos);
        salida.close();
    }

    public Object cargarSerializable(File path) throws FileNotFoundException, IOException, ClassNotFoundException {
    ...
    public void crearArchivoDeTexto(File f, String contenido) {
        try {
            BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(f), "Windows-1252"));
            bw.write(contenido);
            bw.close();
